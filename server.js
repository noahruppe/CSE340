const path = require('path'); // Add the path module to resolve paths safely
const fs = require('fs'); // File system module to check if the file exists

// Resolve the correct path to the utilities module
const utilitiesPath = path.join(__dirname, '../CSE340/utilities/');

// Check if the path exists before requiring it
if (fs.existsSync(utilitiesPath)) {
  const utilities = require(utilitiesPath);
} else {
  console.error('Error: utilities module not found at path:', utilitiesPath);
  process.exit(1); // Exit the process if the module is missing
}
